...
get_last_file() {
	# sdk / imagebuilder
	__HTML=$(curl -skL https://fw.koolcenter.com/binary/ars2/ | tr -d '\n' | sed -E -e 's/ +/ /g' -e 's/(<tr[^>]*>)/\n\1/g' | grep -Eo '^<tr.*</tr>' | grep "openwrt-${1}")
	__LAST_UPDATE_AT=""
	while read LINE
	do
...
git branch -r | tr -d ' ' | sed -E 's/^origin\///' | while raed BRANCH
do
	git checkout "$BRANCH"
	get_last_file "$BRANCH" && update_source && github_push
done
